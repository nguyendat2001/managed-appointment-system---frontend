      ...
      <div class="modal-body p-0">
        <div class="card card-plain">
          <div class="card-header pb-0 text-left">
            <h3 class="font-weight-bolder text-info text-gradient">Cập nhật Dataset</h3>
            <div v-if="this.success" class="alert alert-success alert-dismissible fade show" role="alert">
              <span class="alert-icon"><i class="ni ni-like-2"></i></span>
              <span class="alert-text"><strong>Success!</strong> Cập nhật Dataset thành công!</span>
              <button type="button" class="btn-close" >
                  <span aria-hidden="true" @click="this.success=false">&times;</span>
              </button>
          </div>
            <p class="mb-0">Nhập thông tin dataset mới</p>
            <!-- <button v-if="!this.newDataset.isActive" type="button" class="mt-2 btn btn-outline-secondary" @click="activeDataset()">inactive</button>
            <button v-else type="button" class="mt-2 btn btn-outline-success" @click="inactiveDataset()">active</button> -->
          </div>
          <div class="card-body">
            <form role="form text-left">
              ...
              <label>Bệnh đã chọn:   <span class="font-weight-bolder "> làm mới: <i class="fa fa-refresh fa-spin" @click="this.refresh()"></i></span></label>
...
export default {
  ...
  methods: {
    async activeDataset(){
        try{
          await DatasetService.active(this.id);
          this.getdataset()
      ...
      async inactiveDataset(){
        try{
          await DatasetService.inactive(this.id);
          this.getdataset()
